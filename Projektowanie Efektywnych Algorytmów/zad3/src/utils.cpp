...
namespace Utils {
    ...
    void saveToCSV(const string& filename, const string& instance, int n, const Config& cfg, const Algorithms::AlgorithmResult& res) {
        ...
        if (!exists) file << "Instancja;N;Metoda_Sasiedztwa;Chlodzenie;Wsp_Chlodzenia;Startowa_T;RNN_Start;Czas[us];Najlepszy_Koszt;Optimum;Blad_Wzgledny[%];Sprawdzonych_Sasiadow\n";
    ...
    void saveDetailsToCSV(const string& filename, const Algorithms::AlgorithmResult& res) {
        ...
        for (const auto& step : res.history) {
            file << step.iteration << ";" << step.timestampUs << ";" << step.currentCost << ";" << step.bestCost << "\n";
        }
        file.close();
    }
}
